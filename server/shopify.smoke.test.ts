...
import { createCart, isShopifyConfigured, listProducts } from "./_core/shopify";
...
describe.skipIf(!configured)("shopify smoke (live)", () => {
  ...
  it(
    "creates a checkout-ready cart from a live available variant",
    { timeout: 30_000 },
    async () => {
      const products = await listProducts({ first: 10 });
      const variant = products.flatMap(product => product.variants).find(item => item.availableForSale);

      expect(variant, "No available Shopify variant was found for a checkout handoff test").toBeTruthy();

      const cart = await createCart([{ variantId: variant!.id, quantity: 1 }]);
      expect(cart.itemCount).toBeGreaterThanOrEqual(1);
